...
public class PlayerController6plates : MonoBehaviour
{
    public float speed = 0;
    public TextMeshProUGUI countText;
    public GameObject winTextObject;
    public string playerTag;
    public static bool gameOver = false; // Variable estática para controlar si un jugador ha ganado
    ...
    void SetCountText()
    {
        ...
        if (count >= 6 && !hasWon && !gameOver) // Contar hasta 6 platos y verificar si el jugador no ha ganado aún y el juego no ha terminado
        {
            hasWon = true;
            gameOver = true; // Marcar el juego como terminado

            if (gameObject.CompareTag(playerTag)) // Verificar si este jugador es el que ha recogido los seis platos
            {
